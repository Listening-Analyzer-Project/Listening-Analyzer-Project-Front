...
import { CanonicalListen } from "../types/listen-types";
...
type RequiredOpts = Required<
  Pick<
    StreamOptions,
    "batchSize" | "skipInvalidLines" | "logEvery" | "minMsPlayed"
  >
>;
...
export async function* parseAndBatch(
  ...
  for (const filePath of filePaths) {
    try {
      ...
      console.warn(
        `[unified-streamer] File not found or not readable: ${filePath} — skipping`
      );
      continue;
    }
    ...
    try {
      switch (fileType) {
        case "spotify-json":
          ...
          break;
        case "canonical-json":
          yield* processJsonFile(
            filePath,
            reqOpts,
            stats,
            validateCanonicalListen
          );
          break;
        case "deezer-excel":
        ...
        default:
          console.warn(
            `[unified-streamer] Unknown file type for ${filePath} — skipping`
          );
      }
    } catch (err) {
    ...
    for await (const line of rl) {
      ...
      try {
        ...
        if (!opts.skipInvalidLines) {
          ...
          stream.destroy();
          throw new Error(
            `Invalid JSON on line ${stats.totalLines} of ${filePath}: ${String(
              err
            )}`
          );
        }
      ...
      if (
        typeof canonical.ms_played !== "number" ||
        canonical.ms_played < opts.minMsPlayed
      ) {
        stats.rejected++;
        continue;
      }
...
async function* processDeezerFile(
  ...
  for await (const deezerBatch of streamDeezerExcelRows(
    filePath,
    opts.batchSize
  )) {
