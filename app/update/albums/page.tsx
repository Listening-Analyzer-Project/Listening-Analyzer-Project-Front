...
import { Search, X, RefreshCcw, ArrowUp, ArrowDown, Disc3, Loader2 } from "lucide-react"
...
interface AlbumAnalytics {
  album_id: number
  album_title: string
  release_date: string | null // Ajout de la date de sortie
  artists: string
...
export default function UpdateAlbumsPage() {
  ...
  const totalPages = Math.ceil(totalAlbums / rowsPerPage)

  const columnSortKeys: { [key: string]: string } = {
    "Écoutes ≥ 30s": "valid_listens",
    "Écoutes < 30s": "invalid_listens",
    "Nom de l'Album": "album_title",
    Artistes: "artists",
    "Date de sortie": "release_date", // Ajout de la colonne "Date de sortie"
  }
  ...
  const fetchAlbums = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/analytics/albums?${params.toString()}`)
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`)
      }
      const data: AlbumAnalytics[] = await response.json()

      const totalCountHeader = response.headers.get("X-Total-Count")
      if (totalCountHeader) {
        setTotalAlbums(Number(totalCountHeader))
      } else {
  ...
  const handleSort = useCallback(
    (columnKey: string) => {
      if (sortColumn === columnKey) {
        ...
        // Par défaut, tri ascendant pour les colonnes de texte et de date
        setSortDirection(
          columnKey === "album_title" || columnKey === "artists" || columnKey === "release_date" ? "asc" : "desc",
        )
      }
      setCurrentPage(1)
    },
  ...
  const renderPaginationControls = () => (
    <div className="flex flex-col items-center gap-2 text-sm text-muted-foreground">
      <div className="flex items-center gap-2 text-center">
        ...
        <span>-</span>
        <span>
          Affichage de {(currentPage - 1) * rowsPerPage + 1} à {Math.min(currentPage * rowsPerPage, totalAlbums)} sur{" "}
          {totalAlbums} albums totaux
        </span>
      </div>
    ...
    <div className="container mx-auto py-8">
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <div className="flex flex-col">
            <CardTitle className="text-2xl font-bold flex items-center gap-2">
              <Disc3 className="h-6 w-6 text-gray-800" />
              Update des données par albums
            </CardTitle>
            <CardDescription className="text-gray-500 ml-[32px] mt-1">
              Gérez vos données d'écoute Spotify. ({totalAlbums} albums)
            </CardDescription>
          </div>
      ...
      <div className="flex items-center gap-2 mb-6 p-2 bg-white rounded-lg shadow-sm border">
        ...
        <span className="text-sm text-muted-foreground whitespace-nowrap">
          {albums.length} / {totalAlbums} albums
        </span>
      </div>
                        ...
                        sortKey === "valid_listens"
                          ...
                          : sortKey === "invalid_listens"
                            ...
                            : sortKey === "album_title"
                              ...
                              : sortKey === "artists"
                                ? "w-[180px]" // Ajusté
                                : sortKey === "release_date" // Nouvelle largeur pour la date de sortie
                                  ? "w-[100px]"
                                  : ""
                      }`}
                ...
                {albums.map((album) => (
                  <TableRow key={album.album_id}>
                    <TableCell className="w-[60px] py-1">
                      <span className="text-xs font-medium text-gray-700">{album.rank}</span>
                    </TableCell>
                    <TableCell className="w-[100px] py-1">
                      <span className="text-xs font-medium text-green-600">{album.valid_listens.toLocaleString()}</span>
                    </TableCell>
                    <TableCell className="w-[100px] py-1">
                      <span className="text-xs text-red-600">{album.invalid_listens.toLocaleString()}</span>
                    </TableCell>
                    <TableCell className="w-[200px] py-1">
                      <TruncatedTextWithTooltip text={album.album_title} className="text-xs font-medium" />
                    </TableCell>
                    <TableCell className="w-[180px] py-1">
                      <TruncatedTextWithTooltip text={album.artists} className="text-xs text-gray-600" />
                    </TableCell>
                    <TableCell className="w-[100px] py-1">
                      <TruncatedTextWithTooltip text={album.release_date || "N/A"} className="text-xs text-gray-600" />
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
