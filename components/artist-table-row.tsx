...
import { Save, XCircle, Search } from "lucide-react"
...
export const ArtistTableRow = React.memo(
  ({ artist, countries, loading, onSave, onModifiedChange }: ArtistTableRowProps) => {
    const [localSelectedCountryId, setLocalSelectedCountryId] = useState<number | null>(artist.country_id)
    ...
    return (
      <TableRow key={artist.artist_id}>
        ...
        <TableCell className="w-[100px] py-1">
          <span className="text-xs font-medium text-green-600">{artist.valid_listens.toLocaleString()}</span>
        </TableCell>
        <TableCell className="w-[100px] py-1">
        ...
        <TableCell className="w-[160px] py-1">
          {countries.length > 0 ? ( // Render Select only when countries are loaded
            ...
            >
              <SelectTrigger
                className={cn("h-7 text-xs", isModified && "bg-purple-100 text-purple-800 border-purple-300")}
              >
            ...
            <div className="h-7 flex items-center text-xs text-muted-foreground">Chargement pays...</div> // Placeholder pendant le chargement
          )}
        </TableCell>
        <TableCell className="w-[160px] py-1">
          <TruncatedTextWithTooltip text={artist.spotify_genres || "N/A"} className="text-xs text-gray-600" />
        </TableCell>
        <TableCell className="w-[100px] py-1">
          <div className="flex gap-1">
            ...
            {isModified && (
              <Button size="icon" variant="ghost" onClick={handleLocalCancel} disabled={loading} className="h-7 w-7">
