...
import { ColumnMapping, DeezerListen } from '../../../../types/imports-types'
...
function createColumnMapping(headers: string[]): ColumnMapping {
  ...
  if (missingEssential.length > 0) {
    throw new Error(
      `Missing essential columns: ${missingEssential.join(', ')}. Available columns: ${headers.join(
        ', '
      )}`
    )
  }
...
function mapExcelRowToDeezerListen(row: any, mapping: ColumnMapping): DeezerListen {
  const getValue = (colNumber: number | undefined): any => {
    ...
    try {
      ...
      if (row.values && Array.isArray(row.values) && row.values.length > colNumber) {
...
export async function* streamDeezerExcelRows(
  ...
  try {
    ...
    if (!sheetInfo) {
      throw new Error(`No sheet with Deezer listening history found in ${filePath}`)
    }
    ...
    console.log(`[deezer-utils] Processing Deezer Excel with column mapping:`, mapping)
    ...
    console.log(`[deezer-utils] Missing optional columns: ${getMissingColumns(mapping, headers)}`)
  } finally {
  ...
  for await (const worksheet of workbook) {
    ...
    if (firstRow && firstRow.values) {
      ...
      if (typeof firstRow.eachCell === 'function') {
        ...
        Object.keys(firstRow.values).forEach(k => {
          ...
          if (!isNaN(idx)) {
            ...
            if (val !== null && val !== undefined) headers[idx - 1] = String(val).trim()
          }
        })
      }
      ...
      const hasAllRequired = requiredColumns.every(requiredCol =>
        cleanedHeaders.some(header => header.toLowerCase().includes(requiredCol.toLowerCase()))
      )
